...
def lysis_metric_cal(
    y_true, y_pred, onehot_species_list, species_type, onehot_lysis_list, lysis_type
):
    if species_type == "Human":
        if lysis_type == "HC50":
            idx = [
                i
                for i in range(len(onehot_lysis_list))
                if (str(onehot_species_list[i]) == "[1. 0. 0. 0. 0. 0.]")
                & (str(onehot_lysis_list[i]) == "[1. 0. 0.]")
            ]
...
def csv_best_lysis_metric_record(
    best_target_list, best_predict_list, best_species_list, best_lysis_list
):
    lysis_list, mse_list, pcc_list, ktc_list, ccc_list, r2_list = [], [], [], [], [], []
    hc_list = ["HC5", "HC10", "HC50"]
    for hc in hc_list:
        lysis_list.append(hc)
        mse, pcc, ktc, ccc, r2 = csv_lysis_metric_cal(
            best_target_list,
            best_predict_list,
            best_species_list,
            "Human",
...
def best_lysis_metric_record(
    ...
    for hc in hc_list:
        lysis_list.append(hc)
        mse, pcc, ktc, ccc, r2 = lysis_metric_cal(
            best_target_list,
            best_predict_list,
            best_species_list,
            "Human",
...
def predict(pretrain_model_path, model_path, input_csv_path, output_csv_path):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            predict_pHC, cls_embedding = model(
    ...
    test_df["predicted_HC"] = [
        10 ** (-item) for item in test_predict_list
    ]  # transform pHC value to HC
